...
class Game:
	...
	timer=None
        ...
        def stop(self,c,e,msg):
                if self.game_on==True:
			access = Access_Lists()
			if access.isHigh(c,e,msg) or access.isSuper(c,e,msg):
	                        self.game_on=False
        	                self.spots = []
                	        self.spotsTaken = 0
                                ...
                                if self.game_mode in [self.modes["conquest"],self.modes["rush"]]:
                                        ...
                                        if len(msg.split()) == 2:
                                                ...
                                                if (team.lower() in ['def','defence']) and ('' in self.spots[:4]):
                                                        ...
                                                        c.notice(e.nick,"That team is full")
                                                        return
				if (self.spotsTaken < 8 and self.game_mode == self.modes["squad"]) or (self.spotsTaken < 16 and self.game_mode in [self.modes["conquest"],self.modes["rush"]]):
                                	while not added:
                                        	pos = randint(a, b)
                                        	if self.spots[pos] == "":
	...
	def cmode(self,c,e,msg):
		if len(msg.split(" ")) >= 2:
			if self.game_on==True:
				access = Access_Lists()
				if access.isHigh(c,e,msg) == True or access.isSuper(c,e,msg) == True:
...
def main():
        ...
        if len(sys.argv) > 1:
                ...
                bot = BF3Pickup("#battlefield3.dev", "BF3Pickup_", "za.shadowfire.org", 6667)
        try:
                bot.start()
