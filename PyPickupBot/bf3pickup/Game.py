...
class Game:
        ...
        def start(self,c,e,msg):
                if self.game_on==False:
                        ...
                        if len(msg.split(" ")) == 2:
                                ...
                                if not servSelected or not mapSelected or not modeSelected:
                                        c.notice(e.nick,message+").")
                        if self.game_mode in ["conquest","rush"]:
                                self.spots = ["",]*16
                                self.game_defence = "\x034US: \x0314( " + " ) ( ".join(self.spots[:8]) + " )"
                                self.game_attack = "\x0312RU: \x0314( " + " ) ( ".join(self.spots[8:]) + " )"
                        elif self.game_mode=="squad":
                                self.spots = ["",]*8
                                self.game_defence = "\x034US: \x0314( " + " ) ( ".join(self.spots[:4]) + " )"
                                self.game_attack = "\x0312RU: \x0314( " + " ) ( ".join(self.spots[4:]) + " )"
                        self.do_topic(c,e)
                else:
                ...
                if self.game_on==True:
                        if e.nick in self.spots:
                                ...
                                if self.game_mode in ["conquest","rush"]:
                                        ...
                                        if len(msg.split()) == 2:
                                                team = msg.split()[1]
                                                if team.lower() not in ['us', 'ru']:
                                                        c.notice(e.nick,"Usage: !add [RU|US]")
                                                        return
                                                if (team.lower() == 'us') and ('' in self.spots[:8]):
                                                        a = 0
                                                        b = 7
                                                elif (team.lower() == 'ru') and ('' in self.spots[8:]):
                                                        a = 8
                                                        b = 15
                                                else:
                                        ...
                                        if len(msg.split()) == 2:
                                                ...
                                                if team.lower() not in ['us', 'ru']:
                                                        c.notice(e.nick,"Usage: !add [RU|US]")
                                                        return
                                                if (team.lower() == 'us') and ('' in self.spots[:4]):
                                                        a = 0
                                                        b = 3
                                                elif (team.lower() == 'ru') and ('' in self.spots[4:]):
                                                        a = 4
                                                        b = 7
                                                else:
                                                        c.notice(e.nick,"That team is full")
        ...
        def gameinfo(self,c,e):
                if self.game_on==True:
                        ...
                        c.privmsg(e.target,"Status: No Game")
        ...
        def lastgame(self,c,e):
                ...
                ac.close()
                c.privmsg(e.target,"Last game: Map: %s | Server: %s %s | Admin: %s | Date/Time: %s |" % (" ".join(last_map)," ".join(last_server)," ".join(last_server_IP)," ".join(last_admin)," ".join(last_time)))
        ...
        def cadmin(self,c,e,msg):
                if len(msg.split(" ")) >= 2:
                        if self.game_on==True:
                                access = Access_Lists()
                                if (access.isHigh(c,e,msg) == True) and (access.isHighName(msg.split(" ")[1])):
                                        self.game_admin=msg.split(" ")[1]
                                        self.do_topic(c,e)
                                elif (access.isSuper(c,e,msg) == True) and (access.isSuperName(msg.split(" ")[1])):
                                        self.game_admin=msg.split(" ")[1]
                                        self.do_topic(c,e)
                                else:
        ...
        def saveLastGame(self,server,ip,map,admin,attack,defence):
        ...
        def showRules(self,c,e):
                c.notice(e.nick,"Join the server when I tell you to.")
                c.notice(e.nick,"If you need to leave, call for a sub.")
                c.notice(e.nick,"Do not leave the server until the sub has joined.")
                c.notice(e.nick,"Don't mess around eg TKing.")
                c.notice(e.nick,"Do not go afk for more than 2 minutes without calling a sub")
                c.notice(e.nick,"Type !weapons to see disallowed weapons")

        def showWeapons(self,c,e):
                c.notice(e.nick,"\x02Disallowed weapons and weapon attachments:\x02")
                c.notice(e.nick,"IRNV Optics / Tactical Flashlight / Laser Sight / Mortar")
       
        def begin(self,c,e):
                if self.game_mode in ["conquest","rush"] and self.spotsTaken==16:
                        ...
                        for block in self.spots:
                                if block:
                                        c.privmsg(block,"Please join the following server: %s with password: gamezone" % self.game_server)
                        ...
                        self.game_on=False
                        self.setNews(c,e,self.getNews())
                        self.saveLastGame(self.game_server,self.game_server_IP,self.game_map,self.game_admin,self.game_attack,self.game_defence)
                        self.spots = []
                elif self.game_mode == "squad" and self.spotsTaken==8:
                        c.privmsg(e.target,"\x034The player pool is full. Please make your way to the server and join the relevant Teamspeak Channels.")
                        c.privmsg(e.target,"\x034Players will be notified with the current game information (Server URL and Password)")
                        c.privmsg(self.channel,self.game_defence)
                        c.privmsg(self.channel,self.game_attack)
...
class BF3Pickup(SingleServerIRCBot):
        ...
        def do_command(self, e, msg):
                ...
                if e.command == "die":
                        ...
                        access = Access_Lists()
                        c.notice(e.nick,"Commands: !add , !rem , !moveme , !game , !lastgame , !teams , !lastteams, !servers")
                        if access.isHigh(c,e,msg) == True:
                                c.notice(e.nick,"Admin commands: !sg <map_name> <server> <mode>, !cg, !forcerem <name>")
                                c.notice(e.nick,"!forcemove <name>, !swap <name1> <name2>, !map <map_name>, !server <server_name>")
                                c.notice(e.nick,"!kick <name> <reason>, !kickban <name> <reason(WITH DATE TO UNBAN)>")
                else:
